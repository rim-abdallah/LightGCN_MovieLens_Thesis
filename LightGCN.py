...
from evaluator.python.evaluate_foldout import f1_score
from sampling_utils import get_fixed_sample

from visualization.visualize_lightgcn_bipartite import visualize_lightgcn_bipartite
from visualization.visualize_user_ranking import show_user_top_items
from visualization.export_scores_matrix import export_scores_matrix
from visualization.visualize_interest_graph import visualize_interest_graph_by_distance
from item_popularity import export_item_popularity
#from scores_to_probabilities import export_probability_matrix
#from LightGCN_movieLens.community.louvain_output_communities import louvain_from_probability_output
##from LightGCN_movieLens.community.community_popularity import (
  #  export_community_item_popularity,
   # export_users_inside_communities
#)
from visualization.visualize_louvain_communities import (
    visualize_louvain_communities
)

cpus = [x.name for x in device_lib.list_local_devices() if x.device_type == 'CPU']
data_generator = Data(path=args.data_path + args.dataset,batch_size=1024)
...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    loss_loger, pre_loger, rec_loger, ndcg_loger, f1_loger, hit_loger = [], [], [], [], [], []
    stopping_step = 0
    should_stop = False
    ...
    for epoch in range(1, args.epoch + 1):
        ...
        train_f1 = [
            f1_score(ret['precision'][i], ret['recall'][i])
            for i in range(len(ret['precision']))
        ]
        
        perf_str = 'Epoch %d: train==[%.5f=%.5f + %.5f + %.5f], recall=[%s], precision=[%s], f1=[%s], ndcg=[%s]' % \
                   (epoch, loss, mf_loss, emb_loss, reg_loss, 
                    ', '.join(['%.5f' % r for r in ret['recall']]),
                    ', '.join(['%.5f' % r for r in ret['precision']]),
                    ', '.join(['%.5f' % r for r in train_f1]),
                    ', '.join(['%.5f' % r for r in ret['ndcg']]))
        print(perf_str)
        ...
        train_writer.add_summary(summary_test_loss, epoch // 2)
        t2 = time()
        users_to_test = list(data_generator.test_set.keys())
        ret = test(sess, model, users_to_test, drop_flag=True)
        ...
        train_writer.add_summary(summary_test_acc, epoch // 2)
    ...
    final_perf = "Best Iter=[%d]@[%.1f]\trecall=[%s], precision=[%s], f1=[%s], ndcg=[%s]" % \
...
'''
sampled_users, sampled_items = get_fixed_sample(
    data_generator,
    num_users=4,
    num_items=10
)
...
visualize_interest_graph_by_distance(
    ...
    sampled_users,
    sampled_items,
    graph_name="sample_users"
)

selected_user = sampled_users[0]
...
export_scores_matrix(
    sess=sess,
    model=model,
    sampled_users=[0, 1, 2, 3, 4],
    output_dir="output_scores",
    
)
probs = export_probability_matrix(sess, model)'''

'''item_popularity = export_item_popularity(probs)

user_communities = louvain_from_probability_output(
    probs,
    top_k_neighbors=20
)

export_community_item_popularity(probs, user_communities)

export_users_inside_communities(user_communities)

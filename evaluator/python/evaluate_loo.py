...
#This evaluates your model when:

#👉 Each user has ONLY ONE test item
...
def argmax_top_k(a, top_k=50):#returns top-K recommended items
    ele_idx = heapq.nlargest(top_k, zip(a, itertools.count()))
    return np.array([idx for ele, idx in ele_idx], dtype=np.intc)


def hit(rank, ground_truth):#checks if the true item appears in ranking
...
def ndcg(rank, ground_truth):#Gives higher score if item appears early
    last_idx = sys.maxsize
    for idx, item in enumerate(rank):
        if item == ground_truth:
            last_idx = idx
...
def mrr(rank, ground_truth):#Focuses on first correct item
    last_idx = sys.maxsize
    for idx, item in enumerate(rank):
        if item == ground_truth:
            last_idx = idx
...
def eval_score_matrix_loo(score_matrix, test_items, top_k=50, thread_num=None):#Main function that applies all metrics for all users

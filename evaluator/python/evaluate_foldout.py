...
def argmax_top_k(a, top_k=50):#Selects the top-K items with highest predicted scores
...
def mean_average_precision(rank, ground_truth):
...
def ndcg(rank, ground_truth):#Measures ranking quality, gives more value if relevant items appear higher
    len_rank = len(rank)
...
def mrr(rank, ground_truth):#Mean Reciprocal Rank, checks position of first relevant item
...
def eval_score_matrix_foldout(score_matrix, test_items, top_k=50, thread_num=None):#Main function that applies all metrics for all users
    def _eval_one_user(idx):
        ...
        result.extend(mean_average_precision(ranking, test_item))
